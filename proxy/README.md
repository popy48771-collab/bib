...
> **既定の中継が既にアプリに埋め込まれています。** 通常はこの手順は不要です。
> 自分の Cloudflare アカウントで運用したい場合や、既定の中継が止まった場合に
> 以下の手順で自前の中継を立て、アプリの設定で差し替えてください。
...
- **中継先を `ndlsearch.ndl.go.jp` に限定しています。** 第三者に任意のURLを
  踏ませる踏み台にはなりません
- **呼び出し元を限定しています。** URL は公開サイトのJSに載って誰にでも見えるため、
  `ALLOWED_ORIGINS` に無い生成元からの利用は 403 で断ります。
  自前で立てる場合はここを自分のサイトの生成元に書き換えてください。
  ブラウザで直接開く場合(Origin ヘッダ無し)は通します — それは NDL を
  直接叩くのと変わらず、防いでも意味がないためです
