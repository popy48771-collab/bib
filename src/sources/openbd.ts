...
/** openBD は ONIX + 独自 summary を返す。summary だけ使えば足りる */
...
function normalizePubdate(pubdate: string | undefined): string | undefined {
...
function toBibRecord(item: OpenBdItem): BibRecord | null {
  const s = item.summary
  if (!s?.title) return null

  const description = item.onix?.CollateralDetail?.TextContent?.find(
 ...
 */
export async function fetchByIsbns(
  isbns: string[],
  signal?: AbortSignal,
): Promise<Map<string, BibRecord>> {
  const valid = [...new Set(isbns.map((i) => toIsbn13(i)).filter((i): i is string => !!i))]
  const out = new Map<string, BibRecord>()
  ...
  for (let i = 0; i < valid.length; i += CHUNK) {
    ...
    if (!res.ok) throw new Error(`openBD APIエラー: ${res.status}`)
