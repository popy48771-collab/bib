...
import { encodeForVlm, type EncodedImage } from '../lib/image'
...
function normalizeExtracted(raw: unknown): ExtractedSpine[] {
  const books = (raw as { books?: unknown })?.books
  ...
  for (const b of books) {
    ...
    const authors = Array.isArray(rec.authors)
      ? rec.authors.filter((a): a is string => typeof a === 'string').map(tidy).filter(Boolean)
      : []
...
async function extractWithGemini(
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(
    model,
  )}:generateContent?key=${encodeURIComponent(settings.vlmApiKey)}`
  ...
  if (!text) throw new Error('モデルから読み取り結果が返りませんでした。')
...
export function isVlmConfigured(settings: Settings): boolean {
