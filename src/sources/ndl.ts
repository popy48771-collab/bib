...
export class NdlNotConfiguredError extends Error {
  constructor() {
    super(
      'NDLサーチはブラウザから直接呼べません(CORS非対応)。設定画面でプロキシURLを登録してください。',
    )
 ...
 */
export function buildProxiedUrl(proxyUrl: string, targetUrl: string): string {
  const p = proxyUrl.trim()
  if (!p) throw new NdlNotConfiguredError()
  if (p.endsWith('=')) return p + encodeURIComponent(targetUrl)
 ...
 */
function extractIsbnFromItem(item: Element): { isbn13?: string; isbnRaw?: string } {
  const identifiers: { raw: string; typed: boolean }[] = []
  for (const el of Array.from(item.children)) {
    const ln = el.localName || el.nodeName.replace(/^.*:/, '')
    if (ln !== 'identifier') continue
  ...
  for (const id of identifiers) {
    ...
    if (v) return { isbn13: v, isbnRaw: id.raw }
  }
  // 属性が取れなかった場合は値の形で判定
  for (const id of identifiers) {
    const v = extractIsbn13(id.raw)
...
export function parseNdlResponse(xml: string): BibRecord[] {
...
export interface NdlSearchOptions {
  proxyUrl: string
  maxResults?: number
  signal?: AbortSignal
}

async function ndlRequest(params: URLSearchParams, opts: NdlSearchOptions): Promise<BibRecord[]> {
  if (!opts.proxyUrl?.trim()) throw new NdlNotConfiguredError()
  ...
  if (!res.ok) throw new Error(`NDLサーチ APIエラー: ${res.status}`)
...
export async function searchByTitle(
  ...
  if (authors[0]) p.set('creator', tidy(authors[0]))

  const results = await ndlRequest(p, opts)
  if (results.length > 0 || !authors[0]) return results
...
export function isNdlConfigured(proxyUrl: string | undefined): boolean {
