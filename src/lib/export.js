...
export function exportableEntries(entries, includeUnverified = false) {
    return entries.filter((e) => {
        if (e.status === 'excluded')
            return false;
        if (!e.resolved)
            return false;
        if (!includeUnverified && e.status === 'unverified')
            return false;
        return true;
    });
}
const CSV_HEADERS = ['タイトル', '副題', '著者', '出版社', '出版年', 'ISBN', 'シリーズ', '出典', '状態'];
 ...
 * 改行は CRLF (RFC 4180)。
 */
export function toCsv(entries, includeUnverified = false) {
...
export function toMarkdown(entries, includeUnverified = false) {
    ...
    const rows = list.map((e) => {
        const r = e.resolved;
        return `| ${escapePipe(r.title)} | ${escapePipe((r.authors ?? []).join(', '))} | ${escapePipe(r.publisher ?? '')} | ${r.published ?? ''} | ${r.isbn13 ? formatIsbn13(r.isbn13) : ''} |`;
    });
    return [head, sep, ...rows].join('\n') + '\n';
}
/** BibTeX のキーに使える文字だけ残す */
function bibKey(r, index) {
...
export function toBibtex(entries, includeUnverified = false) {
    return (exportableEntries(entries, includeUnverified)
        .map((e, i) => {
        const r = e.resolved;
        const fields = [`  title = {${bibEscape(r.title)}}`];
        if (r.authors?.length)
            fields.push(`  author = {${bibEscape(r.authors.join(' and '))}}`);
        if (r.publisher)
            fields.push(`  publisher = {${bibEscape(r.publisher)}}`);
        if (r.published)
            fields.push(`  year = {${r.published.slice(0, 4)}}`);
        if (r.isbn13)
            fields.push(`  isbn = {${formatIsbn13(r.isbn13)}}`);
        return `@book{${bibKey(r, i)},\n${fields.join(',\n')}\n}`;
    })
        .join('\n\n') + '\n');
}
/** ISBN だけの一覧。カーリル等の蔵書チェックに流す用途 */
export function toIsbnList(entries) {
    const seen = new Set();
...
export function renderExport(format, entries, includeUnverified = false) {
    switch (format) {
        case 'csv':
