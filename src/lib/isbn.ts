 ...
 */
export function toIsbn13(raw: string | undefined | null): string | null {
  if (!raw) return null
  const v = stripIsbn(raw)
  if (isValidIsbn13(v)) return v
  if (isValidIsbn10(v)) return isbn10To13(v)
  return null
}
 ...
 */
export function extractIsbn13(text: string | undefined | null): string | null {
  if (!text) return null
