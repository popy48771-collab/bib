...
/** Dice 係数 (多重集合として扱う) */
export function diceCoefficient(a, b, n = 2) {
    const ga = ngrams(a, n);
    const gb = ngrams(b, n);
 ...
 */
function containmentBonus(query, target) {
    if (!query || !target)
        return 0;
...
/** タイトル同士の類似度 (0..1) */
export function titleSimilarity(queryTitle, targetTitle) {
...
export function authorSimilarity(queryAuthors, targetAuthors) {
