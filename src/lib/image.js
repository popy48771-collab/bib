 ...
 * 既に十分小さければ拡大はしない。
 */
export async function encodeForVlm(blob, maxEdge = MAX_IMAGE_EDGE, quality = 0.9) {
...
export async function makeThumbnail(blob, maxEdge = 640) {
    const bitmap = await loadBitmap(blob);
    try {
        const longEdge = Math.max(bitmap.width, bitmap.height);
        const scale = longEdge > maxEdge ? maxEdge / longEdge : 1;
        ...
        return await new Promise((resolve, reject) => {
            canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('サムネイル生成に失敗しました'))), 'image/jpeg', 0.8);
        });
    }
    finally {
        bitmap.close();
...
/** 画像の実寸を得る */
export async function getDimensions(blob) {
    const bitmap = await loadBitmap(blob);
    try {
