...
import type { BibRecord, BookEntry, ScoredCandidate } from '../types'
import { AUTO_CONFIRM_THRESHOLD } from '../types'

function makeEntry(over: Partial<BookEntry> = {}): BookEntry {
  return {
    id: 'e1',
    photoId: 'p1',
...
describe('mergeNdlResult — 非破壊性', () => {
  ...
  it('Google Books の候補を消さない', () => {
    const withGb: BookEntry = { ...base, candidates: { googleBooks: [{ record: gbRecord, score: 0.95 }] } }
  ...
  it('食い違いがあれば conflict 状態にして差分を残す', () => {
    const conflicting: BibRecord = { ...ndlRecord, isbn13: '9784062748667' }
  ...
  it('冪等: 同じ結果を2回統合しても壊れない', () => {
    const scored: ScoredCandidate[] = [{ record: ndlRecord, score: 0.93 }]
